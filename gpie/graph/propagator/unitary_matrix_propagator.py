...
class UnitaryMatrixPropagator(Propagator):
    """
    ...
    def _set_precision_mode(self, mode: str | UnaryPropagatorPrecisionMode):
        ...
        if mode not in allowed:
            raise ValueError(f"Invalid precision_mode for UnitaryMatrixPropagator: {mode}")
    ...
    def __matmul__(self, wave: Wave) -> Wave:
        if len(wave.event_shape) != 1:
            raise ValueError(f"UnitaryMatrixPropagator only supports 1D wave input. Got: {wave.event_shape}")
