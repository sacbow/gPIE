...
def test_phase_mask_fft_propagator_forward_backward(xp):
    """Test forward/backward passes for PhaseMaskFFTPropagator."""
    ...
    class TestGraph(Graph):
        def __init__(self):
            ...
            output = PhaseMaskFFTPropagator(phase_mask) @ x_wave
            with self.observe():
...
@pytest.mark.parametrize("xp", backend_libs)
def test_phase_mask_fft_propagator_to_backend(xp):
    """Test that phase_mask and phase_mask_conj are transferred between backends."""
    backend.set_backend(xp)
    rng = get_rng(seed=1)
    n = 4
    phase_mask = random_phase_mask((n, n), dtype=xp.complex128, rng=rng)
...
@pytest.mark.parametrize("xp", backend_libs)
def test_phase_mask_fft_propagator_get_sample_for_output(xp):
    ...
    assert xp.allclose(y_sample, ifft2_centered(fft2_centered(sample) * phase_mask))

    # Also confirm that output.set_sample can accept this result
    output.set_sample(y_sample)
    assert xp.allclose(fft2_centered(output.get_sample()), fft2_centered(sample) * phase_mask)


@pytest.mark.parametrize("xp", backend_libs)
def test_phase_mask_fft_propagator_invalid_inputs(xp):
